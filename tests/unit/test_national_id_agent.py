"""Unit tests for the NationalID agent factory.
...
from doc_extractor.agents import national_id as national_id_module
...
def mocked_deps(monkeypatch: pytest.MonkeyPatch) -> dict[str, MagicMock]:
    ...
    load_prompt_mock.return_value = ("NATIONAL ID PROMPT BODY", "0.1.0")

    monkeypatch.setattr(
        national_id_module.VisionModelFactory, "create", create_mock
    )
    monkeypatch.setattr(
        national_id_module.VisionModelFactory, "validate_api_key", validate_mock
    )
    monkeypatch.setattr(national_id_module, "load_prompt", load_prompt_mock)
...
def test_happy_path_returns_agent_with_national_id_schema(
    mocked_deps: dict[str, MagicMock],
) -> None:
    agent = create_national_id_agent()

    assert isinstance(agent, Agent)
    assert agent.output_schema is NationalID
    assert agent.instructions == ["NATIONAL ID PROMPT BODY"]
    mocked_deps["load_prompt"].assert_called_once_with("national_id")


def test_sonnet_model_is_wired(mocked_deps: dict[str, MagicMock]) -> None:
    """National IDs need precise mask-preservation + embedded-encoding cross-checks."""
    agent = create_national_id_agent()
...
def test_provider_override_beats_yaml(mocked_deps: dict[str, MagicMock]) -> None:
    create_national_id_agent(provider="openai")
...
def test_model_override_beats_yaml(mocked_deps: dict[str, MagicMock]) -> None:
    ...
    create_national_id_agent(model="claude-haiku-4-5-20251001")
...
def test_no_module_level_agent_attribute() -> None:
    ...
    public_attrs = {a for a in dir(national_id_module) if not a.startswith("_")}
    for name in public_attrs:
        value = getattr(national_id_module, name)
