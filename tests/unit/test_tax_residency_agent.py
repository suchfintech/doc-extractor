"""Unit tests for the TaxResidency agent factory."""
...
from doc_extractor.agents import tax_residency as tax_residency_module
...
def mocked_deps(monkeypatch: pytest.MonkeyPatch) -> dict[str, MagicMock]:
    ...
    load_prompt_mock = MagicMock(return_value=("TAX RESIDENCY PROMPT BODY", "0.1.0"))

    monkeypatch.setattr(
        tax_residency_module.VisionModelFactory, "create", create_mock
    )
    monkeypatch.setattr(
        tax_residency_module.VisionModelFactory, "validate_api_key", validate_mock
    )
    monkeypatch.setattr(tax_residency_module, "load_prompt", load_prompt_mock)
...
def test_happy_path_returns_agent_with_tax_residency_schema(
    mocked_deps: dict[str, MagicMock],
) -> None:
    agent = create_tax_residency_agent()

    assert isinstance(agent, Agent)
    assert agent.output_schema is TaxResidency
    assert agent.instructions == ["TAX RESIDENCY PROMPT BODY"]
    mocked_deps["load_prompt"].assert_called_once_with("tax_residency")


def test_sonnet_model_is_wired(mocked_deps: dict[str, MagicMock]) -> None:
    """Per-jurisdiction TIN format preservation (NZ IRD / US SSN / HK / SG / CN)
...
def test_provider_override_beats_yaml(mocked_deps: dict[str, MagicMock]) -> None:
    create_tax_residency_agent(provider="openai")
...
def test_model_override_beats_yaml(mocked_deps: dict[str, MagicMock]) -> None:
    ...
    create_tax_residency_agent(model="claude-haiku-4-5-20251001")
...
def test_no_module_level_agent_attribute() -> None:
    public_attrs = {a for a in dir(tax_residency_module) if not a.startswith("_")}
    for name in public_attrs:
        value = getattr(tax_residency_module, name)
