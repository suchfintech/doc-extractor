"""Unit tests for the DriverLicence agent factory.
...
so Agno's isinstance(model, Model) check passes (convention from worker-2's
...
from doc_extractor.agents import driver_licence as driver_licence_module
...
def mocked_deps(monkeypatch: pytest.MonkeyPatch) -> dict[str, MagicMock]:
    ...
    load_prompt_mock.return_value = ("DRIVER LICENCE PROMPT BODY", "0.1.0")

    monkeypatch.setattr(
        driver_licence_module.VisionModelFactory, "create", create_mock
    )
    monkeypatch.setattr(
        driver_licence_module.VisionModelFactory, "validate_api_key", validate_mock
    )
    monkeypatch.setattr(driver_licence_module, "load_prompt", load_prompt_mock)
...
def test_happy_path_returns_agent_with_driver_licence_schema(
    mocked_deps: dict[str, MagicMock],
) -> None:
    agent = create_driver_licence_agent()

    assert isinstance(agent, Agent)
    assert agent.output_schema is DriverLicence
    assert agent.instructions == ["DRIVER LICENCE PROMPT BODY"]
    mocked_deps["load_prompt"].assert_called_once_with("driver_licence")


def test_sonnet_model_is_wired(mocked_deps: dict[str, MagicMock]) -> None:
    """driver_licence is an ID-class document — same Sonnet default as Passport."""
    agent = create_driver_licence_agent()
...
def test_provider_override_beats_yaml(mocked_deps: dict[str, MagicMock]) -> None:
    create_driver_licence_agent(provider="openai")
...
def test_model_override_beats_yaml(mocked_deps: dict[str, MagicMock]) -> None:
    ...
    create_driver_licence_agent(model="claude-haiku-4-5-20251001")
...
def test_no_module_level_agent_attribute() -> None:
    ...
    public_attrs = {a for a in dir(driver_licence_module) if not a.startswith("_")}
    for name in public_attrs:
        value = getattr(driver_licence_module, name)
