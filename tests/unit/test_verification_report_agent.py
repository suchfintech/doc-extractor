"""Unit tests for the VerificationReport agent factory."""
...
from doc_extractor.agents import verification_report as verification_report_module
...
def mocked_deps(monkeypatch: pytest.MonkeyPatch) -> dict[str, MagicMock]:
    ...
    load_prompt_mock = MagicMock(return_value=("VERIFICATION REPORT PROMPT BODY", "0.1.0"))

    monkeypatch.setattr(
        verification_report_module.VisionModelFactory, "create", create_mock
    )
    monkeypatch.setattr(
        verification_report_module.VisionModelFactory,
        "validate_api_key",
        validate_mock,
    )
    monkeypatch.setattr(verification_report_module, "load_prompt", load_prompt_mock)
...
def test_happy_path_returns_agent_with_verification_report_schema(
    mocked_deps: dict[str, MagicMock],
) -> None:
    agent = create_verification_report_agent()

    assert isinstance(agent, Agent)
    assert agent.output_schema is VerificationReport
    assert agent.instructions == ["VERIFICATION REPORT PROMPT BODY"]
    mocked_deps["load_prompt"].assert_called_once_with("verification_report")


def test_sonnet_model_is_wired(mocked_deps: dict[str, MagicMock]) -> None:
    """Outcome normalisation across NZ EIV / in-person / third-party formats."""
    agent = create_verification_report_agent()
...
def test_provider_override_beats_yaml(mocked_deps: dict[str, MagicMock]) -> None:
    create_verification_report_agent(provider="openai")
...
def test_model_override_beats_yaml(mocked_deps: dict[str, MagicMock]) -> None:
    ...
    create_verification_report_agent(model="claude-haiku-4-5-20251001")
...
def test_no_module_level_agent_attribute() -> None:
    public_attrs = {a for a in dir(verification_report_module) if not a.startswith("_")}
    for name in public_attrs:
        value = getattr(verification_report_module, name)
