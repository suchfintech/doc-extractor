"""Unit tests for the PEP_Declaration agent factory."""
...
from doc_extractor.agents import pep_declaration as pep_declaration_module
...
def mocked_deps(monkeypatch: pytest.MonkeyPatch) -> dict[str, MagicMock]:
    ...
    load_prompt_mock = MagicMock(return_value=("PEP DECLARATION PROMPT BODY", "0.1.0"))

    monkeypatch.setattr(
        pep_declaration_module.VisionModelFactory, "create", create_mock
    )
    monkeypatch.setattr(
        pep_declaration_module.VisionModelFactory, "validate_api_key", validate_mock
    )
    monkeypatch.setattr(pep_declaration_module, "load_prompt", load_prompt_mock)
...
def test_happy_path_returns_agent_with_pep_declaration_schema(
    mocked_deps: dict[str, MagicMock],
) -> None:
    agent = create_pep_declaration_agent()

    assert isinstance(agent, Agent)
    assert agent.output_schema is PEP_Declaration
    assert agent.instructions == ["PEP DECLARATION PROMPT BODY"]
    mocked_deps["load_prompt"].assert_called_once_with("pep_declaration")


def test_sonnet_model_is_wired(mocked_deps: dict[str, MagicMock]) -> None:
    """Box-tick discipline + free-text contradiction handling needs Sonnet."""
    agent = create_pep_declaration_agent()
...
def test_provider_override_beats_yaml(mocked_deps: dict[str, MagicMock]) -> None:
    create_pep_declaration_agent(provider="openai")
...
def test_model_override_beats_yaml(mocked_deps: dict[str, MagicMock]) -> None:
    ...
    create_pep_declaration_agent(model="claude-haiku-4-5-20251001")
