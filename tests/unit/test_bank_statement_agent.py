"""Unit tests for the BankStatement agent factory."""
...
from doc_extractor.agents import bank_statement as bank_statement_module
...
def mocked_deps(monkeypatch: pytest.MonkeyPatch) -> dict[str, MagicMock]:
    ...
    load_prompt_mock = MagicMock(return_value=("BANK STATEMENT PROMPT BODY", "0.1.0"))

    monkeypatch.setattr(
        bank_statement_module.VisionModelFactory, "create", create_mock
    )
    monkeypatch.setattr(
        bank_statement_module.VisionModelFactory, "validate_api_key", validate_mock
    )
    monkeypatch.setattr(bank_statement_module, "load_prompt", load_prompt_mock)
...
def test_happy_path_returns_agent_with_bank_statement_schema(
    mocked_deps: dict[str, MagicMock],
) -> None:
    agent = create_bank_statement_agent()

    assert isinstance(agent, Agent)
    assert agent.output_schema is BankStatement
    assert agent.instructions == ["BANK STATEMENT PROMPT BODY"]
    mocked_deps["load_prompt"].assert_called_once_with("bank_statement")


def test_sonnet_model_is_wired(mocked_deps: dict[str, MagicMock]) -> None:
    """Multi-page header-search discipline + verbatim mask handling needs Sonnet."""
    agent = create_bank_statement_agent()
...
def test_provider_override_beats_yaml(mocked_deps: dict[str, MagicMock]) -> None:
    create_bank_statement_agent(provider="openai")
...
def test_model_override_beats_yaml(mocked_deps: dict[str, MagicMock]) -> None:
    ...
    create_bank_statement_agent(model="claude-haiku-4-5-20251001")
...
def test_no_module_level_agent_attribute() -> None:
    public_attrs = {a for a in dir(bank_statement_module) if not a.startswith("_")}
    for name in public_attrs:
        value = getattr(bank_statement_module, name)
